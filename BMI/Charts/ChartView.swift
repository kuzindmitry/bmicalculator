...
//  BMI
//
//  Created by Dmitry Kuzin on 27/08/2019.
...
class ChartView: UIView {
    ...
    private func setupView() {
        ...
        let color = UIColor(red: 144/255, green: 202/255, blue: 249/255, alpha: 1)
    ...
    private func drawCurvedChart() {
        ...
        if let path = CurveAlgorithm.shared.createCurvedPath(dataPoints) {
            let lineLayer = CAShapeLayer()
            lineLayer.path = path.cgPath
            lineLayer.strokeColor = UIColor(red: 61/255, green: 146/255, blue: 200/255, alpha: 1).cgColor//UIColor.white.cgColor
            lineLayer.fillColor = UIColor.clear.cgColor
    ...
    private func maskGradientLayer() {
        if let dataPoints = dataPoints,
            ...
            maskLayer.fillColor = UIColor(red: 61/255, green: 146/255, blue: 200/255, alpha: 1).cgColor//UIColor.white.cgColor
    ...
    private func drawLables() {
        if let dataEntries = dataEntries,
            ...
            for i in 0..<dataEntries.count {
                ...
                let path = UIBezierPath()
                let x = lineGap*CGFloat(i)
                path.move(to: CGPoint(x: x, y: 0))
                path.addLine(to: CGPoint(x: x, y: gridLayer.frame.size.height))
                let lineLayer = CAShapeLayer()
                lineLayer.path = path.cgPath
                lineLayer.fillColor = UIColor.clear.cgColor
...
//                mainLayer.addSublayer(lineLayer)
    ...
    private func drawHorizontalLines() {
        ...
        for i in 0..<dataEntries.count {
            let path = UIBezierPath()
            let x = lineGap*CGFloat(i) - lineGap/2 + 40
            path.move(to: CGPoint(x: x, y: 0))
            path.addLine(to: CGPoint(x: x, y: gridLayer.frame.size.height))
            let lineLayer = CAShapeLayer()
            lineLayer.path = path.cgPath
